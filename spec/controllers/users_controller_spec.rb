...
RSpec.describe UsersController, type: :controller do
  ...
  describe "POST/users/create" do
    let(:create_user) { User.create(username: 'user', password: 'password1', email: 'user@email.com') }
    ...
    it "creates a user" do
      post :create, params: { user: { username: "user", password: "password1", email: 'user@email.com' } }
    ...
    it 're-renders the page if username is not unique' do
      ...
      post :create, params: { user: { username: "user", password: "password1", email: 'user@email2.com' } }
      expect(response).to have_http_status(200)
    end
